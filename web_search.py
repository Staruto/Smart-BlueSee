...
_DETERMINISTIC_DATE_TIME_HINTS = (
    "what date is today",
    "today's date",
    "what day is today",
    "what time is it",
...
def classify_query_intent(user_text: str) -> str:
    query = user_text.lower().strip()
    if not query:
        return "general_static"

    if any(h in query for h in _DETERMINISTIC_DATE_TIME_HINTS):
        return "deterministic_local"

    if any(h in query for h in _CAMPUS_HINTS):
        return "campus_fact"
...
def should_use_kb(user_text: str) -> bool:
    intent = classify_query_intent(user_text)
    if intent == "campus_fact":
        return True
    return ENABLE_KB_FOR_GENERAL
...
def should_trigger_web_search(user_text: str, kb_stats: dict | None = None) -> tuple[bool, str]:
    ...
    if intent == "deterministic_local":
        return False, "local_deterministic"

    if any(h in query for h in _COMMAND_HINTS):
        return True, "explicit_search_hint"

    if intent == "general_realtime":
    ...
    if kb_stats:
        ...
        if intent == "campus_fact" and (sections == 0 or context_chars < 200):
...
def _normalize_items(payload: dict) -> list[WebSearchItem]:
    ...
    for row in organic:
        ...
        confidence = 0.65
        if any(source.endswith(suf) for suf in (".edu", ".edu.cn")):
            confidence += 0.25
        elif any(source.endswith(suf) for suf in (".gov", ".gov.cn")):
            confidence += 0.2

        if any(pref in source for pref in WEB_SEARCH_PREFERRED_DOMAINS):
            confidence += 0.1

        if any(sd in source for sd in _SOCIAL_DOMAINS):
            confidence -= WEB_SEARCH_SOCIAL_PENALTY
    ...
    seen: set[str] = set()
    for item in items:
        if any(blocked in item.source for blocked in WEB_SEARCH_BLOCKED_DOMAINS):
            continue
...
def maybe_web_search(query: str, kb_stats: dict | None = None) -> tuple[list[WebSearchItem], str]:
    ...
    if not results:
        return [], "search_empty"
    if results[0].confidence < WEB_SEARCH_MIN_ACCEPTED_SCORE:
        return [], "search_low_quality"
